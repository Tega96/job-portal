...
export const getJobs = async (req, res) => {
    const {
        keyword, location, category, type, minSalary, maxSalary, userId,
    } = req.body
    
    const query = {
        isClosed: false,
        ...(keyword && {title: {regex: keyword, $options: 'i'}}),
        ...(location && {location: {regex: locatioin, $options: 'i'}}),
        ...(category && {category}),
        ...(type && {type}),
    };
...
export const getJobsEmployer = async (req, res) => {
    try {
        ...
        const jobs = await Job.find({ company: userId })
            ...
            .lean(); // .lean() will make the jobs plain JS objects so we can add new fields
...
export const getJobById = async (req, res) => {
    try {
        ...
        const job = await Job.find(req.params.id.populate(
            "company",
            "name companyName companyLogo"
        ))

        if (!job) {
            return res.status(404).jsong({ message: "Job not found" })
        }
...
export const deleteJob = async (req, res) => {
    try {
        const job = await Job.findById(req.params.id);
        if (!job) return res.status(404).jsong({ message: "Job not found" })
